...
const TemplateList = ({ searchInput }: { searchInput: string }) => {
  ...
  useEffect(() => {
    if(searchCategory === 'All'){
      ...
      const filteredTemplates = contentTemplates.filter((item) => item.category === searchCategory)
  ...
  useEffect(() => {
    if(searchInput && searchInput.length > 2){
      const filteredTemplates = contentTemplates.filter((item) => item.name.toLocaleLowerCase().includes(searchInput.toLocaleLowerCase()));
  ...
  return (
    <div className='grid grid-cols-1 md:grid-cols-3 gap-4 mx-5 mt-5'>
        {
          templateList.map((template, _) => (
            <div key={template.slug} className="">
